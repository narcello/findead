...
@test 'Test used component' {
  run echo "$(./findead.sh ./tests/components/{components,imports} | grep -o 'No unused components found')"
...
@test 'Test unused component(commented imports)' {
  run echo "$(./findead.sh ./tests/{components,imports_commented} | grep -o '10 possible dead components :/')"
  assert_output "10 possible dead components :/"
}
