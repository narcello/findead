...
searchImports() {
  for COMPONENT in ${COMPONENTS[@]}; do
    ...
    ABSOLUTE_COMPONENT_FILE_PATH="/$(basename "$COMPONENT_FILE_PATH" | cut -d . -f 1)"

    GREP_RECURSIVE_RESULT=$(echo $FIND_RETURN | xargs grep "import.*$COMPONENT_NAME.*from")
    GREP_RECURSIVE_RESULT_FOR_LAZY_IMPORT=$(echo $FIND_RETURN | xargs grep "lazy(() => import(.*$ABSOLUTE_COMPONENT_FILE_PATH.*)")

    COMPONENTS_OCURRENCES=$([[ -z "$GREP_RECURSIVE_RESULT" && -z "$GREP_RECURSIVE_RESULT_FOR_LAZY_IMPORT" ]] && echo 0 || (printf "$GREP_RECURSIVE_RESULT\n$GREP_RECURSIVE_RESULT_FOR_LAZY_IMPORT" | wc -l))
    COMMENTED_IMPORT_OCCURRENCES=$(printf "$GREP_RECURSIVE_RESULT\n$GREP_RECURSIVE_RESULT_FOR_LAZY_IMPORT" | grep // | wc -l)
